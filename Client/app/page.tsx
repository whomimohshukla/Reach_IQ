...
export default function HomePage() {
  ...
  return (
    <main className="workspace-grid min-h-screen overflow-hidden">
      <nav className="mx-auto flex max-w-[1240px] items-center justify-between px-6 py-6 md:px-10">
        <LogoWithText href="/" />
        <div className="hidden items-center gap-7 md:flex">
          {navLinks.map((link) => <Link key={link.label} href={link.href} className="text-sm text-muted-foreground transition-colors hover:text-foreground">{link.label}</Link>)}
        </div>
      ...
      <section className="mx-auto grid max-w-[1240px] items-center gap-14 px-6 pb-24 pt-20 md:grid-cols-[0.92fr_1.08fr] md:px-10 md:pb-32 md:pt-28">
        <div>
          ...
          <h1 className="max-w-2xl text-[clamp(4rem,8vw,7.8rem)] font-semibold leading-[0.86] tracking-[-0.09em]">Turn chats<br /><span className="text-[#70bfb5]">into customers.</span></h1>
        ...
        <div className="relative border border-border bg-card/70 p-4 md:p-5">
          ...
          <div className="grid gap-4 py-5 sm:grid-cols-[1fr_1.2fr]">
            ...
            <div className="border border-border bg-[#18382f] p-5 text-[#e6f0e8]"><div className="flex items-center justify-between"><span className="font-mono text-[9px] uppercase tracking-[0.15em] text-[#f07856]">AI recommendation</span><Sparkles className="h-4 w-4 text-[#c8ee9e]" /></div><p className="mt-6 text-xl font-semibold leading-tight">Follow up with Aarav now.</p><p className="mt-3 text-sm leading-relaxed text-[#9bb7a9]">He asked about installation pricing and has a 92% conversion score.</p><button className="mt-8 flex w-full items-center justify-between border border-[#6d9387] px-3 py-3 text-left text-xs text-[#c8ee9e] hover:bg-[#295044]">Open conversation <ArrowRight className="h-4 w-4" /></button></div>
          </div>
